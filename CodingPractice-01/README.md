...
제곱을 계산하는 `Square` 정적 메서드를 만들고, 대리자에 할당하여 `calc(5)`를 출력하세요.
...
`delegate (int x) { return x * x; }`로 제곱, `delegate (int x) { return x * x * x; }`로 
세제곱 익명 메서드를 만드세요. `"3의 제곱: {square(3)}"`과 `"3의 세제곱: {cube(3)}"`을 
출력하세요.
...
익명 메서드를 만들고, `"안녕하세요!"`와 `"익명 메서드를 사용 중입니다."`를 전달하여 
호출하세요.
...
`Action<string>`에 `delegate (string msg) { Console.WriteLine($"[LOG] {msg}"); }`를 
...
매개변수를 생략한 익명 메서드를 할당하고, `onClick(null, EventArgs.Empty)`로 호출하여 
`"클릭 이벤트 발생!"`을 출력하세요.
